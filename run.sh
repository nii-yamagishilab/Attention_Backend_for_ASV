...
root=/home/fbmao/fengsong/kaldi/egs/cnceleb/v1

. /home/fbmao/fengsong/kaldi/egs/cnceleb/v1/cmd.sh
# . /home/fbmao/fengsong/kaldi/egs/cnceleb/v1/path.sh
. path.sh
set -e
mfccdir=/groups1/gcc50479/kaldi/egs/cnceleb/v2/mfcc
vaddir=/groups1/gcc50479/kaldi/egs/cnceleb/v2/mfcc

cnceleb1_root=/groups1/gcc50479/cnceleb/CN-Celeb
cnceleb2_root=/groups1/gcc50479/cnceleb/CN-Celeb2
eval_trials_core=$root/data/eval_test/trials/trials.lst
musan_root=/groups1/gcc50479/spk/data/musan
nnet_dir=$root/exp
...
if [ $stage -le 3 ]; then
  ...
  cp -rf data/train_aug data/train_aug_600k
...
if [ $stage -le 4 ]; then
  ...
  # cp -rf data/train data/train_combined
...
if [ $stage -le 11 ]; then
  ...
  # sort -n -t ' ' -k 2 exp/Thu_Mar_11_12:53:57_2021/backend_scores.txt | cut -d' ' -f3 | cut -d'[' -f2 | cut -d']' -f1 > temp
  # cut -d' ' -f1,2 $root/exp/scores/iv_eval_scores | paste -d' ' - temp > $root/exp/scores/cnceleb_eval_scores
  # sort -n -t ' ' -k 2 exp/Thu_Mar_11_12:53:57_2021/backend_scores.txt | cut -d' ' -f3 | cut -d'[' -f2 | cut -d']' -f1 > temp
  cut -d' ' -f3 exp/$1/backend_scores.txt > temp
  n=`wc -l temp | cut -d' ' -f1`
  echo "$n"
  cut -d' ' -f1,2 $root/exp/scores/iv_eval_scores | head -${n} | paste -d' ' - temp > $root/exp/scores/cnceleb_eval_scores
  head -${n} $eval_trials_core > trials
  eval_trials_core=trials
  echo -e "\nCN-Celeb Eval Core:";
  eer=$(paste $eval_trials_core $nnet_dir/scores/cnceleb_eval_scores | awk '{print $6, $3}' | compute-eer - 2>/dev/null)
  mindcf1=`$root/sid/compute_min_dcf.py --p-target 0.01 $nnet_dir/scores/cnceleb_eval_scores $eval_trials_core 2>/dev/null`
  mindcf2=`$root/sid/compute_min_dcf.py --p-target 0.001 $nnet_dir/scores/cnceleb_eval_scores $eval_trials_core 2>/dev/null`
  echo "EER: $eer%"
